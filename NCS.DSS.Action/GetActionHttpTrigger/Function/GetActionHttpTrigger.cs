...
namespace NCS.DSS.Action.GetActionHttpTrigger.Function
{
    public class GetActionHttpTrigger
    {
        ...
        {
            _logger.LogTrace("Function {FunctionName} has been invoked", nameof(GetActionHttpTrigger));
            ...
            _logger.LogTrace("Attempting to retrieve Actions for Customer. Customer GUID: {CustomerGuid}", customerGuid);
            ...
            if (actions.Count == 1)
            {
                _logger.LogTrace("Action successfully retrieved. Action ID: {ActionId}", actions.FirstOrDefault()!.ActionId);
            ...
            _logger.LogTrace("{Count} Actions successfully retrieved. Action IDs: {ActionIds}", actions.Count, actionsIds);
            _logger.LogTrace("Function {FunctionName} has finished invoking", nameof(GetActionHttpTrigger));
